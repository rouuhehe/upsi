          ...
          <div className="p-6">
            ...
            {reviews.length === 0 ? (
              <div className="text-center py-12">
                ...
                <p className="text-gray-400">Se el primero en dejar una reseña</p>
              </div>
            ) : (
              <div className="space-y-4">
                {reviews.map((review) => (
                  <div key={review.id} className="bg-gray-50 rounded-lg p-5 border border-gray-200 hover:shadow-md transition-shadow duration-200">
...
function InteractiveStarRating({ value, onChange }: { value: number; onChange: (rating: number) => void }) {
  return (
    <div className="flex gap-1">
      {[1, 2, 3, 4, 5].map((i) => (
        <button
          ...
          type="button"
          onClick={() => onChange(i)}
          className="focus:outline-none focus:ring-2 focus:ring-sky-500 focus:ring-offset-2 rounded transition-all duration-200 hover:scale-110"
        >
          <Star
            size={24}
            className={
